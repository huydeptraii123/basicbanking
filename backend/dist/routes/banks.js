...
router.get('/sync-accountIds', async (req, res) => {
    try {
        const banks = await prisma_1.default.bank.findMany({ where: { accountId: null } });
        const updates = [];
        for (const b of banks) {
            console.log('syncing bank id:', b.id, 'sharableId:', b.sharableId);
            if (b.sharableId) {
                const updated = await prisma_1.default.bank.update({ where: { id: b.id }, data: { accountId: b.sharableId } });
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const banks = await prisma_1.default.bank.findMany({ where: { userId } });
    ...
    try {
        ...
        if (base64Regex.test(lookupKey)) {
            try {
                ...
                console.warn('⚠️ Base64 decode failed:', (e === null || e === void 0 ? void 0 : e.message) || e);
            }
        }
        // --- Sanitize fallbacks ---
...
router.get('/:id', async (req, res) => {
    try {
        ...
        // 1️⃣ exact match
        let bank = await prisma_1.default.bank.findUnique({ where: { id } });
...
router.delete('/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const bank = await prisma_1.default.bank.findUnique({ where: { id } });
        if (!bank)
            return res.status(404).json({ error: 'bank not found' });
        await prisma_1.default.bank.delete({ where: { id } });
